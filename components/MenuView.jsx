...
// Pour les boissons en plusieurs contenances : "3,5 / 4,5 / 6" + "25cl / 33cl / 50cl"
...
export default function MenuView({ sections }) {
  ...
  // Panier
  const [cart, setCart] = useState([]); // { key, id, name, price, quantity }
  const [cartOpen, setCartOpen] = useState(false);
  const [picker, setPicker] = useState(null); // item dont on choisit la taille
  ...
  useEffect(() => {
    ...
    const chip = navRef.current.querySelector(`[data-target="${activeId}"]`);
        ...
        <div className="wrap navbar">
          ...
          <div className="catnav-track" ref={navRef}>
            {cats.map((c) => (
              <button
                key={c.slug}
                className={"chip" + (activeId === c.slug ? " active" : "")}
                data-target={c.slug}
                onClick={() => goTo(c.slug)}
              >
                {c.name}
