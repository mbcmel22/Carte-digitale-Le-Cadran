...
export default function Dashboard() {
  ...
  // Graphique : commandes par jour sur les 7 derniers jours
  const chart = useMemo(() => {
    const days = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date();
      ...
      days.push({ date: d, label: DAYS_FR[d.getDay()], count: 0 });
    }
    orders.forEach((o) => {
      const od = new Date(o.created_at);
      od.setHours(0, 0, 0, 0);
      const slot = days.find((x) => x.date.getTime() === od.getTime());
        ...
        <div className="chart-bars">
          {chart.days.map((d, i) => (
            <div className="chart-col" key={i}>
              <div
                className="chart-bar"
                style={{ height: (d.count / chart.max) * 100 + "%" }}
              >
